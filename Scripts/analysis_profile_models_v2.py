...
os.chdir(r'C:\Users\s1995204\Documents_LOCAL\Modeling\1_year\Analysis')
...
for i in filelist:
    ...
    if name == '3D':
    ...
    elif (name == '3D_BHE') or (name == '3D_BHE_x0'):
        ...
        interval3 = np.arange(0,150,0.1)
        interval3 = [round(num, 1) for num in interval3]
        f3 = interp1d(data['ZONE'],data['T'])
        T_3D3 = f3(interval3)
        data_new = pd.DataFrame(list(zip(interval3, T_3D3)), columns =['ZONE', 'T']) 
        data_new['ZONE'].round(decimals=1)
    else:
        data_new = data
...
#ax.plot(data_new['ZONE'], mean,label = 'mean', color='r', lw=3)
ax.plot(df['ZONE'], df['Steady_state'],label = 'Steady State', color='k', lw=3)
ax.plot(df['ZONE'], df['1D_CF'],label = '1D SI', color='c', lw=3)
ax.plot(df['ZONE'], df['1D_CF_BHE'],label = '1D F', color='b', lw=3)
ax.plot(df['ZONE'], df['SOLAR_IC'],'--' , label = '1D Solar IC', color='k', lw=3)
ax.plot(df['ZONE'], df['SOLAR_EF'],'--',label = '1D Solar EF',  color='g', lw=3)


plt.axvline(x=25, ymin=0, ymax=0.55, linewidth=1, color='k', ls='--')
...
ax.set(xlabel="Distance along profile",
       ylabel="Temperature",
       title="a) Temperature along profile after 1 year of production in 1D model")
#ax.set(xlabel="Distance along profile",
#       ylabel="Temperature",
#       title="Temperature along profile after 1 year of production in 1D model")
plt.axis((0,200,0,16))
plt.legend(loc='best')
plt.savefig('Temp_profile1D2.png')
...
ax1.plot(df['ZONE'], df['Steady_state'],'--',label = 'Steady State', color='k', lw=3)
ax1.plot(df['ZONE'], df['3D_BHE'],label = '3D F x = 26.5m', color='b', lw=3)
ax1.plot(df['ZONE'], df['3D_BHE_x0'],label = '3D F x = 0m', color='y', lw=3)
ax1.plot(df['ZONE'], df['3D'],label = '3D SI x = 26.5m', color='c', lw=3)
ax1.plot(df['ZONE'], df['3D_x0'],label = '3D SI x = 0m', color='r', lw=3)

plt.axvline(x=BHEt, ymin=0, ymax=1, linewidth=1, color='k', ls='--')
plt.axvline(x=BHEb, ymin=0, ymax=1, linewidth=1, color='k', ls='--')
#plt.axvline(x=80, ymin=0, ymax=0.06, linewidth=1, color='k', ls='--')
#ax2.tick_params(axis='y', labelcolor=color)
#ax1.set(xlabel="Distance along profile",
...
ax1.set(xlabel="Distance along profile",
       ylabel="Temperature",
       title='b) Temperature along profile after 1 year of production in 3D model')
#ax1.set(xlabel="Distance along profile",
#       ylabel="Temperature",
#       title="Temperature along profile after 1 year of production in 1D model")
#ax2.axis((0,200,1,14))
...
plt.savefig('Temp_profile3D2.png')
