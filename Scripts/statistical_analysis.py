...
df3['mud'] = df2[['clay','seatclay','seatearth', 'fireclay']].sum(axis=1)
df3['sand'] = df2[['sand', 'soil']].sum(axis=1)
df3['fine rock'] = df2[['mudstone','shale','siltstone','clayrock','seatrock']].sum(axis=1)
df3['coarse rock'] = df2[['sandstone', 'conglomerate']].sum(axis=1)
df3['carbonate'] = df2[['dolomite', 'limestone']].sum(axis=1)
df3['coal'] = df2[['coal', 'ribs']].sum(axis=1)
df3['ironstone'] = df2[['ironstone']].sum(axis=1)
